...
##### Habit versus Ellenberg light indicator values -----
...
fitHFvEL_pl <- phylolm(ellenberg_light ~ -1 + habit, model = "OUrandomRoot", 
                       data = stomata, phy = phy, upper.bound = 1e10)
summary(fitHFvEL_pl)

# therefore using standard ANOVA
fitHFvEL <- lm(ellenberg_light ~ habit, data = stomata)
aovHFvEL <- anova(fitHFvEL)

# for plotting confidence intervals
fitHFvEL <- lm(ellenberg_light ~ -1 + habit, data = stomata)

hf <- c("tree", "shrub", "perennial", "biennial", "annual")

pdf(str_c(path_figures, "/figure_hf-light.pdf"), 4, 7, useDingbats = FALSE)
...
hs <- tapply(stomata$ellenberg_light, stomata$habit, hist, plot = F, breaks = 0:9)  
for (i in 1:5) {
  rect(0:8, rep(i - 1, 9), 1:9, i - 1 + hs[[hf[i]]]$density, col = "black", 
       border = "white", lwd = 2)
}

# label life forms
s <- sprintf("%s (%s)", hf, table(stomata$habit)[hf])
text(0 + 0.5 * strwidth(s), 1:5, labels = s, pos = 1)

# axes
axis(1, at = 0.5:8.5, labels = 1:9, lwd = 0, line = -1.5)
for (i in 0:4) {
  axis(2, at = i + seq(0.2, 0.6, 0.2), labels = seq(0.2, 0.6, 0.2), lwd = 0, 
       lwd.ticks = 1)
}
...
ci <- confint(fitHFvEL_pl)
ci <- ci[str_c("habit", hf), ] # reorder
mu <- coef(fitHFvEL_pl)[paste0("habit", hf)]
for (i in 1:5) {
...
pdf(str_c(path_figures, "/figure_lf-light.pdf"), 4, 7, useDingbats = FALSE)
par(mai = c(1.5, 1, 0, 0.25), cex.lab = 1, las = 1)
plot(0, 0, type = "n", xlim = c(0, 9), ylim = c(0, 5), xlab = "", ylab = "", axes = F)  
