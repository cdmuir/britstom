...
##### Growth form versus Ellenberg light indicator values -----
...
fitGFvEL_pl <- phylolm(ellenberg_light ~ -1 + growthform, model = "OUrandomRoot", 
                       data = stomata, phy = phy, upper.bound = 1e10)
summary(fitGFvEL_pl)

# therefore using standard ANOVA
fitGFvEL <- lm(ellenberg_light ~ growthform, data = stomata)
aovGFvEL <- anova(fitGFvEL)

# for plotting confidence intervals
fitGFvEL <- lm(ellenberg_light ~ -1 + growthform, data = stomata)

gf <- c("tree", "shrub", "perennial", "biennial", "annual")

pdf(str_c(path_figures, "/figure_gf-light.pdf"), 4, 7, useDingbats = FALSE)
...
hs <- tapply(stomata$ellenberg_light, stomata$growthform, hist, plot = F, breaks = 0:9)  
for (i in 1:5) {
  rect(0:8, rep(i - 1, 9), 1:9, i - 1 + hs[[gf[i]]]$density, col = "black", 
       border = "white", lwd = 2)
}

# label life forms
s <- sprintf("%s (%s)", gf, table(stomata$growthform)[gf])
...
ci <- confint(fitGFvEL_pl)
ci <- ci[str_c("growthform", gf), ] # reorder
mu <- coef(fitGFvEL_pl)[paste0("growthform", gf)]
for (i in 1:5) {
