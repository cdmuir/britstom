...
fitSR_gf_a <- phylolm(sr_even ~ growthform * ellenberg_light, model = "OUrandomRoot",
                      data = stomata, phy = phy, upper.bound = 10)
fitSR_gf_b <- phylolm(sr_even ~ growthform + ellenberg_light, model = "OUrandomRoot",
                      data = stomata, phy = phy, upper.bound = 10)
fitSR_gf_c <- phylolm(sr_even ~ ellenberg_light, model = "OUrandomRoot",
                      data = stomata, phy = phy, upper.bound = 10)
fitSR_gf_d <- phylolm(sr_even ~ growthform, model = "OUrandomRoot",
                      data = stomata, phy = phy, upper.bound = 10)
fitSR_gf_e <- phylolm(sr_even ~ 1, model = "OUrandomRoot",
                      data = stomata, phy = phy, upper.bound = 10)
aicSR_gf <- sapply(list(fitSR_gf_a, fitSR_gf_a, fitSR_gf_a, fitSR_gf_a, fitSR_gf_a), AIC)
...
gf <- data.frame(growthform = c("tree", "shrub", "perennial", "biennial", "annual"))
gf %<>% 
  expand(growthform, growthform) %>%
  remove_diag()
gf %<>% mutate(mean = numeric(nrow(.)),
              median = numeric(nrow(.)),
              lower = numeric(nrow(.)),
              upper = numeric(nrow(.)))
...
for (i in 1:nrow(gf)) {
  ...
  s1 <- fitSR_gf_pl$bootstrap[, sprintf('growthform%s', gf$growthform[i])]
  s2 <- fitSR_gf_pl$bootstrap[, sprintf('growthform%s', gf$growthform1[i])]
  gf[i, 3:6] <- compare_slopes(s1, s2)
...
fitSR_lf_b <- phylolm(sr_even ~ lifeform + ellenberg_light, model = "OUrandomRoot",
                  data = stomata, phy = phy, upper.bound = 10)
fitSR_lf_c <- phylolm(sr_even ~ ellenberg_light, model = "OUrandomRoot",
                  data = stomata, phy = phy, upper.bound = 10)
fitSR_lf_d <- phylolm(sr_even ~ lifeform, model = "OUrandomRoot",
                  data = stomata, phy = phy, upper.bound = 10)
fitSR_lf_e <- phylolm(sr_even ~ 1, model = "OUrandomRoot",
                  data = stomata, phy = phy, upper.bound = 10)
aicSR_lf <- sapply(list(fitSR_lf_a, fitSR_lf_b, fitSR_lf_c, fitSR_lf_d, fitSR_lf_e), AIC)
